...
st.set_page_config("AI IMAGE GENERATOR", "./favicon.ico")
st.markdown("<h1 style='color: #A2CFFE; font-family: sans-serif; text-align: center'>AI IMAGE GENERATOR</h1>", unsafe_allow_html=True)

if not hasattr(st, "global_image_generated_counter"):
    st.global_image_generated_counter = 0

if "user_API_key" in st.session_state:
    st.session_state.user_API_key = "r8_DpWXTdEWVHj5esqBK0VIRlSnoUdZkcn2dR68B"
...
if option is not None:
    ...
    if api_key_option == "Community API key":
        with st.expander("Community Credits Progress Bar", expanded=True):
            st.write(" (check the limit below) or use your own Replicate API key in the sidebar by selecting 'Use your own API key'.")
            st.progress(st.global_image_generated_counter, f"'AI IMAGE GENERATOER' can generate {200-st.global_image_generated_counter} more images")
        ...
        if st.session_state.user_API_key is not None: 
            if not re.match("^r8_", st.session_state.user_API_key) or not len(st.session_state.user_API_key) > 30:
                st.sidebar.error("Please enter a correct API key.")   
